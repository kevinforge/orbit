...
如果你已经在本机使用 Claude Code、Codex 或 CodeBuddy，直接复制下面的提示发给它：

```text
请帮我使用 npm 安装 Orbit，执行远程包安装命令：
npm install -g @kevinforge/orbit
然后运行 orbit，并确认 http://localhost:4317 可以访问。不要安装公开 npm
上那个无关的 orbit 包。
```
...
协作提示词对所有团队都遵守依赖关系：独立任务可以并行，存在依赖的任务必须等待前置结果。普通对话始终只面向一位员工。
