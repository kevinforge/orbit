...
Runtimes advertise native slash commands through the ACP `available_commands_update` session notification. The ACP runtime accepts this one update type before a run settles — including the creation window between `session/new` and its response, whose notifications are buffered per session id and flushed once the id is assigned — and converts each frame into `AgentCommand` entries (`name`, `description`, optional `inputHint`); entries with empty names or descriptions are dropped. `AgentSession` keeps the latest snapshot per runtime session in memory, publishes it as an `agent.commands.updated` event for the open page, and clears the cache when a settled session id differs from the cached one (resume degraded to a replacement session), so a new session re-announces from scratch. Command snapshots are never persisted to disk; `/api/state` exposes them as `agentCommands` so a refresh restores the current view. The composer shows a slash-command menu for the conversation's current target employee: an explicit `@员工:` marker routes to that employee in every interaction mode (matching assignment routing), and without a marker the target follows plain conversation routing — the last directly-addressed employee in direct mode, none in collaborative or supervised mode — while the `/` trigger is mutually exclusive with `@` mention detection. A missing snapshot is probed once through `POST /api/agents/probe-commands` when the menu opens; the probe short-circuits on an announced snapshot, adopts its result into the authoritative cache only while no real announcement exists, and broadcasts neither its result nor its failure once a real announcement has landed, so a late probe frame can never overwrite live session commands. Sending a `/command` submits only the raw message text: the server derives whether it resolves to a native command, its target employee, and the exact command text through the shared `resolveSlashSendTarget` (interaction mode + `@员工:` marker + the authoritative snapshot), so the channel history and the runtime execution share one source and cannot diverge; `RunManager` runs a derived command through the same serialized queue and delivers the command text to the runtime unchanged, skipping `<orbit-context>` prompt assembly and follow-up routing, while the raw message text stays in the channel history like any user message.
...
Clicking an entry opens the read-only file preview panel on the right side of the shell (issue #165); `Enter`/`Space` are equivalent. The panel (`src/ui/FilePreviewPanel.tsx`) fetches `GET /api/local-path/preview?path=…`, which classifies the file by extension whitelist: text and markdown previews embed the first 1 MB of decoded content (UTF-8 with BOM stripping and replacement characters for non-UTF-8 bytes), images and PDFs render through `GET /api/local-path/preview/raw` with the browser's native `<img>`/`<iframe>` decoding, and every other kind shows a hint with a locate action. Both endpoints (`src/server/local-path-preview.ts`) reuse `resolveRevealTarget` from the reveal flow — `~` expansion, `:line`/`:line:col` suffix stripping, `fs.realpath`, and the workspace boundary check — and raw serving maps exact Content-Types from an extension whitelist (never `text/html`), refuses non-image/non-PDF kinds, caps raw files at 32 MB, and always sends `nosniff`. Serving PDFs inline is a deliberate relaxation that applies only to the preview routes; attachment downloads keep their force-download policy. The panel is a single panel — clicking another file replaces its content — and collapses into a fixed overlay on viewports narrower than 920px. Text previews render with syntax highlighting and a sticky line-number gutter (`src/ui/code-highlight.ts` registers a curated highlight.js language set and maps extensions deterministically; unknown extensions stay plain text), and the panel width is drag-resizable from its left edge with the value persisted in `localStorage` (`orbit.previewWidth`) and shared by the third grid column and the overlay through the `--preview-width` custom property.
