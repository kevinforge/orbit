...
Runtimes advertise native slash commands through the ACP `available_commands_update` session notification. The ACP runtime accepts this one update type before a run settles — including the creation window between `session/new` and its response, whose notifications are buffered per session id and flushed once the id is assigned — and converts each frame into `AgentCommand` entries (`name`, `description`, optional `inputHint`); entries with empty names or descriptions are dropped. `AgentSession` keeps the latest snapshot per runtime session in memory, publishes it as an `agent.commands.updated` event for the open page, and clears the cache when a settled session id differs from the cached one (resume degraded to a replacement session), so a new session re-announces from scratch. Command snapshots are never persisted to disk; `/api/state` exposes them as `agentCommands` so a refresh restores the current view. The composer shows a slash-command menu for the conversation's current target employee — the last directly-addressed employee in direct mode, the composer-selected employee in collaborative mode, and no native commands in supervised mode — and the `/` trigger is mutually exclusive with `@` mention detection. Sending a `/command` carries a native delivery flag through `POST /api/messages`, where the server validates the target employee and the command name against the advertised list before enqueueing; `RunManager` runs it through the same serialized queue but delivers the raw text to the runtime unchanged, skipping `<orbit-context>` prompt assembly and follow-up routing, while the raw command text stays in the channel history like any user message.
