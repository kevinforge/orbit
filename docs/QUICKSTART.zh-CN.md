...
Orbit 是一个本地优先的数字员工工作台。左侧默认会显示这些数字员工：产品经理（pm）、架构师（architect）、开发（developer）、测试（tester）、协调员。你把本地项目目录交给 Orbit，然后在聊天框里用固定的 `@id:` 把任务分配给不同数字员工：
...
| 运行时 | 常见用途 | 安装方式 |
| --- | --- | --- |
| Claude Code | 开发、协调 | `npm install -g @anthropic-ai/claude-code` |
| OpenAI Codex | 产品、架构、通用任务 | `npm install -g @openai/codex` |
| CodeBuddy | 测试或自定义员工 | `npm install -g @tencent-ai/codebuddy-code` |

当某个运行时缺失时，Orbit 会在数字员工设置面板里显示同一条安装命令。你可以从页面复制命令，在终端里安装对应 CLI，按该 CLI 自己的提示完成登录或授权，然后回到 Orbit 重新检测运行环境。
...
选择「多数字员工协作」时，Orbit 会默认启用「架构师（architect）」「开发（developer）」「测试（tester）」「协调员」，并把它们配置到当前可用的运行时上。选择「空白工作区」则不会自动启用数字员工，你需要自己配置。

协作模板还会要求协调员按依赖关系顺序派活：前置数字员工完成后，再指派后续数字员工；互不依赖的工作仍可并行指派。
...
1. 点击「数字员工」旁边的 `+`。
2. 在弹窗里打开需要的数字员工开关。
3. 新手建议至少启用「开发（developer）」；如果想要协作流程，再启用「架构师（architect）」「测试（tester）」和「协调员」。
4. 检查每个数字员工的「运行时」是否显示可用。
5. 点击「保存」。

如果 Orbit 提示某个运行时未安装，先复制页面上显示的安装命令，或者打开旁边的安装链接来安装对应 CLI。安装或登录完成后，回到 Orbit 点击「重新检测运行环境」。
...
如果你启用了协调员，也可以直接描述目标，让协调员帮助分配：

```text
请帮我给这个项目增加一个登录页的表单校验，要求先评估方案，再实现，再测试。
```

点击侧栏底部的「协作洞察」，可以查看已完成与进行中的任务、参与数字员工、执行时间轴和端到端耗时。
...
只需要检查你实际使用的运行时即可。如果命令不存在，就使用 Orbit 为该运行时显示的安装命令。安装或登录完成后，回到 Orbit 点击「重新检测运行环境」。
