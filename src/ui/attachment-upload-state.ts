 ...
 * 用单调递增的上下文版本把每次上传绑定到发起时的会话快照：
 * - 切换工作区或会话时 `resetContext()` 递增版本并清空上传计数，旧请求
...
export function createAttachmentUploadLifecycle() {
  ...
  return {
    /** 捕获当前上下文快照并占用一个上传槽位。 */
    beginUpload(context: { workspaceId: string; conversationId: string }): AttachmentUploadContext {
      uploadingCount += 1;
