...
test("repository exposes open source contribution and release guidance", () => {
  ...
  const releaseNotes = readRepoFile("docs/RELEASE_NOTES_v1.0.0.md");
  ...
  assert.match(releaseDecisions, /Trusted Publishing/);
  assert.match(releaseDecisions, /id-token: write/);
  assert.match(releaseDecisions, /free of long-lived npm registry secrets/);
  assert.match(releaseDecisions, /npm publish --dry-run --access public --ignore-scripts --provenance/);
  ...
  assert.match(releaseChecklist, /Trusted Publishing/);
  assert.match(releaseChecklist, /id-token: write/);
  assert.match(releaseChecklist, /does\s+not require a long-lived `NPM_TOKEN` secret/);
  assert.match(releaseChecklist, /npm publish --dry-run --access public --ignore-scripts --provenance/);
  ...
  assert.match(releaseChecklist, /docs\/RELEASE_NOTES_v1\.0\.0\.md/);
  assert.match(releaseChecklist, /docs\/STABILITY_VERIFICATION\.md/);

  assert.match(releaseNotes, /Status: stable release/);
  ...
  assert.match(releaseNotes, /npm install -g @kevinforge\/orbit@1\.0\.0/);
  ...
  assert.match(releaseNotes, /npm run build/);
  ...
  assert.match(readme, /docs\/RELEASE_NOTES_v1\.0\.0\.md/);
  ...
  assert.match(readiness, /Trusted Publishing/);
  assert.match(readiness, /RELEASE_DECISIONS\.md/);
  assert.match(readiness, /RELEASE_NOTES_v1\.0\.0\.md/);
  ...
  assert.match(support, /Release verification/);
  assert.match(support, /Support Scope For 1\.x/);
