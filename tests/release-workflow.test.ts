...
test("release workflow verifies, creates an npm package for every supported target, then publishes", () => {
  ...
  assert.match(workflow, /npm publish --dry-run --access public --ignore-scripts --provenance/);
  assert.match(workflow, /npm publish --access public --ignore-scripts --provenance/);
